	...
	public static void main(String[] args) throws Exception {
		...
		try (Connection conn = DriverManager.getConnection(url, properties)) {
			String[] preSqls = new String[]{
					...
					"create table test1.t1(id int not null," +
							"c1 boolean," +
							"c2 tinyint," +
							"c3 smallint," +
							"c4 bigint," +
			...
			try (AdbClient client = new AdbClient(config)) {
				TableSchema schema0 = client.getTableSchema("test1.t1");
				...
				System.out.println("select * from test1.t1-----------------------------------");
				client.sql(connection -> {
					try (Statement stat = connection.createStatement()) {
						stat.execute("select * from test1.t1");
